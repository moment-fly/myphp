	var HZ6D_CONFIGS = {
		...
		'talk_host': "http://www22.53kf.com",
		'base_host': "53kf.com",
		'style_id': "1",
		'com_id': "70742861",
		'guest_id': "",
		'kf':'0659,0694,0832,881,0573,899,sz18,sz22,hw03,sz10,0766,0986,sz01,sz15,sz03,sz05,sz06,sz07,sz08,sz09,sz13,sz02,gz03,gz04,gz05,gz06,gz07,gz02,gz09,gz01,sh01,hz03,hz02,hz01,sh05,sh02',
				...
				*/
				if (fav.indexOf('fail') == -1 && fav != '') { // 将fav返回的值赋值到hz6d_fav_companys、hz6d_coms_info中
					...
					for (var i in fav) {
                        ...
                        hz6d_fav_companys[i][0] = html_encode(hz6d_fav_companys[i][0]);
	...
	for(var i in hz6d_fav_companys){
		...
		if(i == "0"){//默认分组
			if (!!document.getElementById('fav_grp_'+i)) {
				flp('#fav_grp_'+i).html('<div class="hz6d_cn"><div class="arrow"></div><div class="hz6d_t"><span>'+html_encode(hz6d_fav_companys[i][0])+'</span>(<em>0</em>)</div><div class="btn_con" style="display:none;"><a class="edit" operating="edit"></a></div></div><ul></ul>');
			} else {
				flp('.mnkf_list_classify_create').before('<div class="mnkf_list_classify" id="fav_grp_'+i+'"><div class="hz6d_cn"><div class="arrow"></div><div class="hz6d_t"><span>'+html_encode(hz6d_fav_companys[i][0])+'</span>(<em>0</em>)</div><div class="btn_con" style="display:none;"><a class="edit" operating="edit"></a></div></div><ul></ul></div>');
			}
		}else{
			if (!!document.getElementById('fav_grp_'+i)) {
				flp('#fav_grp_'+i).html('<div class="hz6d_cn"><div class="arrow"></div><div class="hz6d_t"><span>'+html_encode(hz6d_fav_companys[i][0])+'</span>(<em>0</em>)</div><div class="btn_con" style="display:none;"><a class="edit" operating="edit"></a><a class="del"></a></div></div><ul></ul>');
			} else {
				flp('.mnkf_list_classify_create').before('<div class="mnkf_list_classify" id="fav_grp_'+i+'"><div class="hz6d_cn"><div class="arrow"></div><div class="hz6d_t"><span>'+html_encode(hz6d_fav_companys[i][0])+'</span>(<em>0</em>)</div><div class="btn_con" style="display:none;"><a class="edit" operating="edit"></a><a class="del"></a></div></div><ul></ul></div>');
			}
		}
		var num = 0;
		var online = 0;
...
flp(document).ready(function() {
			...
			if(flp.trim(flp(this).val()) == ''){
				...
				hz6d_sendKhFavData(opts, function(a,b){
					if(a == 1){
						...
						pa.html('<div class="hz6d_cn"><div class="arrow"></div><div class="hz6d_t"><span>'+html_encode(gn)+'</span>(<em>0</em>)</div><div class="btn_con" style="display:none;"><a class="edit" operating="edit"></a><a class="del" operating="del"></a></div></div><ul></ul>');
	...
	flp('.editing').live('blur keydown',function(e){
			...
			if((e.type=='focusout')||(e.type=='keydown'&&e.keyCode==13)){
				...
				hz6d_sendKhFavData(opts, function(a){
					if(a == 1){
						var new_val = html_encode(that.val());					   
						that.parent('span').html(new_val);
					}else{
						hz6d_alert("修改组名失败！");
					}
				});
